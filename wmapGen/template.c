...
#define tester "./test_images.txt"
#define labels "./test_label_list.txt"
...
void test_nn(_nn *nn)
lbrace
    FILE *fp = (FILE *) fopen(tester, "r");
    FILE *flab = (FILE *) fopen(labels, "r");
    assert(fp != NULL && flab != NULL);
    int i, t, m, n, k, iptdim, optdim, preIdx, ans, nCorrect = 0, nTestcase = 0;
    double temp, preVal;
    iptdim = nn->laydim[0];
    ...
    lbrace
        ...
        if(i != 0 && (i + 1) % iptdim == 0) // every single test;
        lbrace
